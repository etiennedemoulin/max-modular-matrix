...
function onList(row, col, gain, time) {
  ...
  import_max_api.default.outlet("tomatrixctl", `/row/${row + 1}/col/${col + 1}`, gain);
}
function onClear() {
...
function onDict(dict) {
  ...
  if (!dict.patch) {
    ...
    dict.connections.forEach((connection) => {
      ...
      import_max_api.default.outlet("tomatrixctl", `/row/${connection.in + 1}/col/${connection.out + 1}`, connection.gain);
    });
  } else {
    if (dict.ramptime) {
      onRamp(dict.ramptime);
    ...
    dict.connections.forEach((connection) => {
      ...
      import_max_api.default.outlet("tomatrixctl", `/row/${inputIndex + 1}/col/${outputIndex + 1}`, connection.gain);
    });
  }
}
function onOpen() {
...
function onPatch(input, output, lin, time) {
  ...
  import_max_api.default.outlet("tomatrixctl", `/row/${inputIndex + 1}/col/${outputIndex + 1}`, lin);
}
...
function generateMatrix() {
  ...
  globals.userMatrix.inputs.forEach((e, i2) => {
    globals.userMatrix.initwith += `/row/${i2 + 1}/name ${e}, `;
  });
  globals.userMatrix.outputs.forEach((e, i2) => {
    globals.userMatrix.initwith += `/col/${i2 + 1}/name ${e}, `;
  });
  globals.routingMatrix.inputs.forEach((e, i2) => {
    globals.routingMatrix.initwith += `/row/${i2 + 1}/name ${e}, `;
    globals.routingMatrix.crosspatch.inputs += `${e} `;
  });
  globals.routingMatrix.outputs.forEach((e, i2) => {
    globals.routingMatrix.initwith += `/col/${i2 + 1}/name ${e}, `;
